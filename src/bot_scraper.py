    ...
    async def login(
        ...
        while not logged_in:
            ...
            if await page.get_by_text(
                ...
                return
            elif page.get_by_text("Incorrect password.").is_visible(timeout=8000):
                await page.locator(".dialog-button", has_text="OK").click()
                await page.get_by_placeholder("Account / Phone number").clear()
                await page.locator("#login-pwd").clear()
                await page.locator('input[name="user-verify"]').clear()
...
# TODO Add a password incorrect stuff incase of wrong password
# TODO Also add an exception incase the user has already done their tasks
